...
void
cmd_tcp(int sockfd) {
    int maxfdp1, nread, nwrite, fd, replycode, file1, localsize, i, set, new_sock;
    char host[16];
    char port[6];
    fd_set rset;
            ...
            if (replycode == USERNAME) {
                ...
                if (strncmp(rbuf, "ls", 2) == 0) {
                    ...
                    if (strlen(rbuf) < 4 || strtok(rbuf, " ") == NULL || (temp = strtok(NULL, "\r\n")) == NULL) {
                        ...
                        if ((setting = atoi(temp)) == 0 || setting < 5 || setting > 2048) {
                            sprintf(wbuf, "Usage: ul <upload_limitation> (5~2048)\nftp->");
                            write(STDOUT_FILENO, wbuf, strlen(wbuf));
                            continue;
                        } else {
                            ul = setting;
            ...
            if (strncmp(rbuf, "213", 3) == 0) {
                ...
                if (action == GETFILE) {
                    ...
                    if (access(dir, F_OK) != 0) {
                        ...
                        localsize = lseek(file1, 0, SEEK_END);
                        if (localsize < size) {
                            sprintf(wbuf, "REST %d\n", localsize);
                            if (write(sockfd, wbuf, strlen(wbuf)) != strlen(wbuf))
                                printf("write error\n");
...
void
ftp_list(int data_sockfd) {
    int		nread;

    for ( ; ; )
    {
        /* data to read from socket */
        if ( (nread = recv(data_sockfd, rbuf1, MAXBUF, 0)) < 0)
            printf("recv error\n");
        else if (nread == 0)
            break;
...
ftp_get(int sck, char *pDownloadFileName_s) {
    ...
    if (action == GETFILE1) {
        localsize = lseek(file, 0, SEEK_END);
    }
    ...
    for (;;) { // read message
        ...
        //write to file
        if (write(file, str1, nread) != nread)
            printf("Error --- Can not send to file.\n");
        gettimeofday(&tv2, NULL);
        localsize += nread;
        totalsize += nread;
        secsize += nread;
        percent = (int) (100 * (double) localsize / size);
        count++;
...
int active(int sckfd) {
    ...
    *ip_4 = '\0';
    ip_4++;
    memset(wbuf, 0, MAXBUF);
    //sprintf(wbuf, "PORT 10,128,220,104,100,0\n");
     sprintf(wbuf, "PORT %s,%s,%s,%s,%d,%d\n", ip_1, ip_2, ip_3, ip_4, client_port >> 8, client_port & 0xff);
    return get_sock;
}
